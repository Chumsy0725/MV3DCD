...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, change=None, mask=False, mask_folder=None, alpha_mask_folder=None):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # print(mask_path)
        alpha_mask_path = os.path.join(alpha_mask_folder, extr.name)
    
        change_mask = None
        alpha_mask = None

        if mask and mask_folder:
            mask_path = os.path.join(mask_folder, extr.name)
            change_mask = try_open_image_variants(mask_path)

        if alpha_mask_folder:
            alpha_mask_path = os.path.join(alpha_mask_folder, extr.name)
            alpha_mask = try_open_image_variants(alpha_mask_path)
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              ...
                              pose=pose, change_mask=change_mask, alpha_mask=alpha_mask)
...
def readColmapSceneInfo(path, model_path, images, eval, change=None,mask=None, llffhold=8):
    try:
    ...
    mask_folder = model_path + '/renders/noisy_masks'
    alpha_mask_folder = model_path + '/renders/alpha_masks'
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, 
                                           images_folder=os.path.join(path, reading_dir), change=change, mask=mask, mask_folder=mask_folder, alpha_mask_folder=alpha_mask_folder)
    ...
    if change is not None:
        ...
        print('Loading Change Scene Cameras')
